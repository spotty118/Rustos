...
impl GPUSystem {
    ...
    fn initialize_best_gpu(&mut self) -> Result<(), &'static str> {
        ...
        if let Some(gpu_index) = best_gpu_index {
            ...
            // Initialize GPU compute engine for AI acceleration
            crate::println!("[GPU] Initializing GPU compute engine...");
            match compute::init_compute_engine(gpu) {
                Ok(_) => {
                    crate::println!("[GPU] GPU compute engine initialized successfully");
                    if compute::is_compute_available() {
                        crate::println!("[GPU] GPU compute acceleration is available for AI workloads");
                        compute::print_compute_status();
                    }
                }
                Err(e) => {
                    crate::println!("[GPU] Warning: Failed to initialize compute engine: {}", e);
                    crate::println!("[GPU] AI workloads will fall back to CPU processing");
                }
            }

            // Initialize multi-GPU management if multiple GPUs are detected
            if self.detected_gpus.len() > 1 {
                crate::println!("[GPU] Multiple GPUs detected, initializing multi-GPU management...");
                let gpu_capabilities: heapless::Vec<GPUCapabilities, 8> = self.detected_gpus.clone();
                match multi_gpu::init_multi_gpu_manager(gpu_capabilities.as_slice()) {
                    Ok(_) => {
                        crate::println!("[GPU] Multi-GPU management initialized for {} GPUs", self.detected_gpus.len());
                        multi_gpu::generate_multi_gpu_report().ok();
                    }
                    Err(e) => {
                        crate::println!("[GPU] Warning: Failed to initialize multi-GPU management: {}", e);
                    }
                }
            }
        }
